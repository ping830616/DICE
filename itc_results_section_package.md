...
5. The localization story is practical and hardware-relevant.
...
6. The LLM layer is useful because it is grounded, not because it improves detection.
   The saved scored baseline preserves the dominant tier in all summaries, preserves the dominant mechanism in `90%` of mixed-profile summaries and `100%` of full-profile summaries, and shows `0%` hallucinated claims in both profiles.
...
- earliest abnormal-window timing and one representative localization figure
- diagnosis and hardware hotspot evidence
- cross-workload transfer summary
- one compact LLM-grounding scorecard
...
- complete per-case localization tables
- implementation detail for the scored LLM pipeline
...
3. `Figure 5`: Time to the first abnormal window by stressor and profile.
   Caption: "Earliest abnormal-window timing by stressor. DICE behaves as a time-sensitive monitor: some anomalies surface almost immediately, whereas others require a longer observation window."

4. `Figure 6`: Representative localization traces.
   Caption: "Representative mixed-profile traces showing the first sustained abnormal interval. The highlighted windows illustrate that DICE responds to compact clusters of abnormal evidence rather than isolated score spikes."

5. `Table V`: Diagnosis and localization summary for the selected `Tier-0/1/2` configurations.
   Caption: "Diagnosis quality at the selected operating points. The mixed profile is the stronger deployment-facing diagnosis setting, while both profiles retain strong mechanism-level localization."

6. `Figure 8`: Hardware hotspot summary.
   Caption: "Most visible hardware hotspots in the final DICE head. Evidence concentrates in the unified-memory and swap path and, secondarily, the Neural Engine, which makes the alerts interpretable for real systems."
...
8. LLM grounding scorecard.
   Caption: "Grounded LLM triage summary. The language model improves interpretability by summarizing structured DICE evidence without adding unsupported claims."
...
The current released dataset supports first-warning timing but does not include a ground-truth crash manifest. For that reason, the main paper should not claim quantitative crash prediction from the released data alone. Instead, the correct statement is that DICE already computes the key precursor signal needed for such a study: the first sustained abnormal window. The repository now includes a crash-aware extension that records reproducible crash evidence and pairs it with this first-warning timestamp to measure warning precision, recall before crash, lead-time distribution, and false alarms per monitored hour. Once recollection with crash evidence is complete, this extension can support a rigorous predictive-maintenance result without changing the core DICE detector.
